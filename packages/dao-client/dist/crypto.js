...
export class CryptoUtils {
    ...
    async sign(privateKeyBase64, message) {
        const key = await window.crypto.subtle.importKey('pkcs8', CryptoUtils.base64ToArrayBuffer(privateKeyBase64), { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['sign']);
        const signature = await window.crypto.subtle.sign('RSASSA-PKCS1-v1_5', key, new TextEncoder().encode(message));
    ...
    async verify(publicKeyBase64, payload, signatureBase64) {
        const key = await window.crypto.subtle.importKey('spki', CryptoUtils.base64ToArrayBuffer(publicKeyBase64), { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']);
        return window.crypto.subtle.verify('RSASSA-PKCS1-v1_5', key, CryptoUtils.base64ToArrayBuffer(signatureBase64), new TextEncoder().encode(payload));
    }
    /**
